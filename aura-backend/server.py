...
# import base64
# import time
# from flask import Flask, request, jsonify, session, send_from_directory, send_file, make_response, Response
...
# flask_env = os.environ.get('FLASK_ENV', 'development')  # default dev
# IS_PROD = (
...
# )
# print(f"🔍 Environment: {'Production' if IS_PROD else 'Development'} ({flask_env})")
...
#     print("🎯 API chat stream endpoint hit")
#     try:
#         if not db or not model:
#             return create_json_response({"error": "Backend services not running."}, 500)
...
#         if not session_id:
#             return create_json_response({"error": "sessionId is required"}, 400)
...
#                 for chunk in chat_session.send_message(gemini_content, stream=True):
#                     chunk_text = getattr(chunk, "text", None) or str(chunk) or ""
...
#                     session_ref.update({"last_updated": datetime.utcnow().isoformat()})
...
#                         "timestamp": datetime.utcnow().isoformat()
#                     })
#                     print("✅ Streamed chat saved to Firestore")
#                 except Exception as e:
...
#     sessions = sorted(sessions, key=lambda x: x['last_updated'], reverse=True)
...
#         return jsonify({"error": "sessionId and title required"}), 400
    
#     session_ref = db.collection("users").document(user_id).collection("sessions").document(session_id)
#     session_ref.update({"title": new_title, "last_updated": datetime.utcnow().isoformat()})
