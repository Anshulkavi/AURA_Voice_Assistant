...
// Clean API URL logic - no hardcoded domains
const getApiUrl = () => {
  // Always use relative URLs for same-domain requests
  return "/api"
}
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      setMessages([
        {
          id: 0,
          text: `Connection failed: ${error.message}. Please check if the backend is running properly.`,
          sender: "bot",
        },
      ])
    }
